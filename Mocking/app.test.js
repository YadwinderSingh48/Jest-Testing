// app.test.js
jest.mock('./mathUtils'); // Automatically use the mocks from __mocks__/mathUtils.js  because test directly can't know which function called when so we need to create mock for them. 
...
describe('calculate', () => {
  ...
  test('calls add with correct arguments', () => {
    calculate(1, 2, 'add');
    expect(add).toHaveBeenCalled(); // test that add function would be called
    expect(add).toHaveBeenCalledWith(1, 2); // test that add function must be called with params 1 and 2;
